...
interface EditChoraleModalProps {
  isOpen: boolean
...
export default function EditChoraleModal({ isOpen, chorale, onClose, onSuccess }: EditChoraleModalProps) {
  ...
  useEffect(() => {
    if (chorale) {
      setFormData({
        nom: chorale.nom || '',
        description: chorale.description || '',
        ville: chorale.ville || '',
        pays: chorale.pays || '',
        actif: (chorale.statut === 'actif' || chorale.actif) ?? true,
      })
    }
  }, [chorale])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Succès
      alert('✅ Chorale modifiée avec succès !')
      onSuccess()
      handleClose()
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-900">Modifier la chorale</h2>
          <button
            onClick={handleClose}
